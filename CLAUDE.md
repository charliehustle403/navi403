...
My primary surface is the **Claude Code terminal** on my Windows machine. I
frequently continue that *same* session from the **Claude mobile app** while I'm
away, using Remote Control. I occasionally open the **VS Code extension** just to
have you look over the codebase — it's the least-used surface and is mostly for
reading, not driving long tasks.

Because sessions are often steered from a phone, follow these defaults unless I
say otherwise:

- Keep responses concise and skimmable. Long walls of text are hard to read on mobile.
- Before any large or multi-file change, give a short plan and wait for my go-ahead.
- Never run destructive or irreversible commands without explicit confirmation in
  the chat — this includes force-push, deleting branches/files, history rewrites,
  and `git reset --hard`. Mobile makes accidental taps easy, so always confirm.
- Prefer small, reviewable commits with clear messages. Summarize what changed in
  prose, since I often can't review a full diff on a phone.
- When a long-running task finishes, end with a one-line status summary so the push
  notification is actually useful (e.g. "tests pass, 0 failures" or "build green").
- Some commands only work from the local terminal (`/mcp`, `/plugin`, `/resume`),
  so don't rely on those when I'm on mobile.

## Git workflow

- Single remote is the source of truth across all my devices — commit and push
  regularly so phone, terminal, and any other machine stay consistent.
...
- Branching: **SAP-style transport workflow** — develop on `dev`, test there, then
  ...
  (PRs only) — `bootstrap.ps1` applies this for you. Always ask before destructive
  prod (`main`) operations.
